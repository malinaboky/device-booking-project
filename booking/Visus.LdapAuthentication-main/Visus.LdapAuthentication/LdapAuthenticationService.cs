...
namespace Visus.LdapAuthentication {
    ...
    public sealed class LdapAuthenticationService<TUser>
        ...
        public ILdapUser Login(string username, string password) {
            using var connection = this._options.Connect(this._options.User, this._options.Password,
                this._logger);

            var retval = new TUser();
            var groupAttribs = this._options.Mapping.RequiredGroupAttributes.Where<string>(attr => !string.IsNullOrEmpty(attr)).ToArray();
